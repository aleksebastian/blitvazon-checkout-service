...
const PriceDeliveryAndStock = (props) => {
  ...
  return (
    <div style={{ width: "100%" }}>
        ...
        {props.isPrimeFreeOneDay && (
          <div>
            ...
            <div>
              ...
              <BlueInlineText>FREE Returns &#8744;</BlueInlineText>
            </div>
          </div>
        )}
      </div>
      ...
      {props.isFreeDelivery || props.isPrimeFreeOneDay ? (
        <div style={{ marginBottom: "20px" }}>
          ...
          <GreyText style={smallerPadding}>
            Order within{" "}
            {hours === 1
              ? `${hours} hr and ${Math.round(minTillMidnight)} mins`
              : `${hours} hrs and ${Math.round(minTillMidnight)} mins`}
          </GreyText>
        ...
        <div style={{ marginBottom: "20px" }}>
          <Text style={smallerPadding}>
            Get it in{" "}
            {`${Math.floor(Number(props.price) / 10)} to ${Math.floor(
              Number(props.price) / 10 + 2
            )}`}{" "}
            days
          </Text>
          <GreyText style={smallerPadding}>
      ...
      <Stock
        style={props.inventory ? { color: "#007600" } : { color: "#B12704" }}
      >
        {props.inventory ? "In Stock." : "Currently unavailble."}
      </Stock>
      {!props.inventory ? (
